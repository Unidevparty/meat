...
{	
	...
	public static function defaultConstants()
	{
		return array(
			...
			'ROOT_PATH'							=> __DIR__,
			'NO_WRITES'							=> ( isset( $_SERVER['IPS-CIC'] ) and !isset( $_SERVER['IPS-CIC-ACP'] ) ),
			'DEBUG_LOG'							=> FALSE,
			'LOG_FALLBACK_DIR'					=> '{root}/uploads/logs',
			'STORE_METHOD'						=> ( isset( $_SERVER['IPS-CIC'] ) ? 'Database' : 'FileSystem' ),
			'STORE_CONFIG'						=> '{"path":"{root}/datastore"}',
			'CACHE_METHOD'						=> 'None',
			'CACHE_CONFIG'						=> '{}',
			'CACHE_PAGE_TIMEOUT'				=> ( isset( $_SERVER['IPS-CIC'] ) ? 300 : 30 ),
			'TEST_CACHING'						=> FALSE,
			'EMAIL_DEBUG_PATH'					=> NULL,
			'BULK_MAILS_PER_CYCLE'				=> 200,
			'JAVA_PATH'							=> "",
			...
			'SUITE_UNIQUE_KEY'					=> mb_substr( md5( '5e697' . '$Rev: 3023$'), 10, 10 ),
			'CACHEBUST_KEY'						=> mb_substr( md5( '5e697' . '$Rev: 3023$'), 10, 10 ), // This looks unnecessary but SUITE_UNIQUE_KEY can be set to a constant constant in constants.php whereas we need a version specific constant for cache busting.
