...
{
	public function loadingIconTest()
	{
		...
		$url = \IPS\Http\Url::internal('app=core&module=system&controller=ajax&do=loadingIconTest&type=a');
		\IPS\Output::i()->output .= "<div class='ipsBox'>" . \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->multipleRedirect( $url ) . '</div><br><br>';
		$url = \IPS\Http\Url::internal('app=core&module=system&controller=ajax&do=loadingIconTest&type=b');
		\IPS\Output::i()->output .= "<div class='ipsBox'>" . \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->multipleRedirect( $url ) . '</div><br><br>';
		$url = \IPS\Http\Url::internal('app=core&module=system&controller=ajax&do=loadingIconTest&type=c');
		\IPS\Output::i()->output .= "<div class='ipsBox'>" . \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->multipleRedirect( $url ) . '</div><br><br>';
		$url = \IPS\Http\Url::internal('app=core&module=system&controller=ajax&do=loadingIconTest&type=d');
		\IPS\Output::i()->output .= "<div class='ipsBox'>" . \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->multipleRedirect( $url ) . '</div><br><br>';
		$url = \IPS\Http\Url::internal('app=core&module=system&controller=ajax&do=loadingIconTest&type=a');
		\IPS\Output::i()->output .= "<div class='ipsBox'>" . \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->multipleRedirect( $url, NULL, 250 ) . '</div><br><br>';
		$url = \IPS\Http\Url::internal('app=core&module=system&controller=ajax&do=loadingIconTest&type=b');
		\IPS\Output::i()->output .= "<div class='ipsBox'>" . \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->multipleRedirect( $url, NULL, 250 ) . '</div><br><br>';
		
		\IPS\Output::i()->output .= "<div class='ipsBox ipsLoading ipsFaded ipsPad'>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nam mattis quis diam sed lobortis. Quisque eget ante eros. Aliquam mattis fringilla commodo. Etiam sagittis pulvinar blandit. Curabitur ut dolor pulvinar, molestie dui a, rhoncus est. Curabitur eleifend nunc ac augue tempus accumsan. Nulla placerat diam sed ex tincidunt, in ultrices mauris condimentum. Maecenas auctor tristique eros eu ullamcorper. Donec sed metus ex. Donec a massa et enim eleifend pulvinar. Ut gravida nisi vel dui blandit sollicitudin. Duis molestie, nunc vitae accumsan molestie, dolor dolor euismod risus, a hendrerit leo elit sed leo. Sed cursus vel nisi id vehicula. Praesent suscipit mollis tortor, laoreet lobortis urna varius a. Suspendisse sed turpis neque. In hac habitasse platea dictumst. Nam sagittis ex velit, id rhoncus ipsum viverra in. Nullam vitae accumsan sapien. Aenean nec feugiat mi, quis finibus justo. In tempor magna velit, vitae ornare odio placerat vel. Vivamus sed convallis quam, a cursus nunc.</div>";
	}
	...
	public function usernameExists()
	{
		...
		if ( $result == array( 'result' => 'ok' ) )
		{
			...
			{
				if( preg_match( '/^' . str_replace( '\*', '.*', preg_quote( $bannedName, '/' ) ) . '$/iu', $name ) ) // -IBR- для русских букв в именах
				{
