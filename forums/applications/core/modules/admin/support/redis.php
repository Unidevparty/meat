...
class _redis extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		if ( \IPS\CACHE_METHOD == 'Redis' or \IPS\STORE_METHOD == 'Redis' )
		{
			...
			$datasource[] = $data;
			/*-IBR- статистика по страницам для гостей в кеше редис*/
            $data = array( 'type' => 'redis_output_cache', 'count' => NULL, 'enabled' => false );
            ...
            if ( \IPS\OUTPUT_CACHE_METHOD == 'Redis' )
            {
                ...
                /* Lets ensure we have something */
                $data['count'] = \count( \IPS\Redis::i()->debugGetKeys( \IPS\Redis::i()->get( 'redisKeyOutputCache' ) . '_*', TRUE ) );
            }
