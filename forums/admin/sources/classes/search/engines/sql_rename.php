 ...
 * IP.Board v3.4.8
 * Global Search
 * Last Updated: $Date: 2014-09-29 13:29:26 -0400 (Mon, 29 Sep 2014) $
 * </pre>
 *
...
class search_engine
{
	...
	public function formatSearchTerm( $search_term )
	{
		...
		if( $isBoolean )
		{
            ...
            $search_term = str_replace( "&gt;", ">"	, $search_term );
            $search_term = str_replace( "&lt;",  "<", $search_term );//<script нам не страшен, его заменили при parseCleanValue
		}
		else
		{
			$search_term	= str_replace( "&quot;"   , ''    ,  $search_term );
        ...
        if (preg_match_all( '/(?:^|\s+)(img|quote|code|html|javascript|a href|script|color|span|div|border|style)(?:\s+|$)/', $search_term, $removed_search_terms ))
        {
            if (count($removed_search_terms[1])) $removedTerms = array_merge($removedTerms, $removed_search_terms[1][0]);
            $search_term	= preg_replace( '/(^|\s+)(?:img|quote|code|html|script|javascript|a href|color|span|div|border|style)(\s+|$)/', str_replace( "  ", " ", "$1$2" ), $search_term );
        }

        if ($isBoolean) {
            //разбиваем строку на слова
            if (preg_match_all('/(".+?")/', $search_term, $immune)) { //сначала извлекаем неизменяемые фразы в кавычках
                $search_term = preg_replace('/".+?"/', 'specialspacerforquotedimmunephrases', $search_term); //исключаем неизменяемые фразы в кавычках
            }
            str_replace('"', '', $search_term); //зачищаем от одиноких и пустых кавычек
            preg_match_all('/([+\-~<>\(]+||^)([^+\-~<>\)\*]+?)($|[\*\)]+|\s)/', $search_term, $_words); //делим запрос на слова и операторы
		...
		{
			if ( IPSText::mbstrlen( $_w ) >= $this->settings['min_search_word'] OR strstr($_suffix[$k],'*') )
			{
                /* Check for disallowed search terms */
                if (preg_match_all( '/(?:^|\s+)(img|quote|code|html|javascript|a href|script|color|span|div|border|style)(?:\s+|$)/', $_w, $removed_search_terms ))
                {
                    if (count($removed_search_terms[1]))
                        $removedTerms = array_merge($removedTerms, $removed_search_terms[1][0]);
                    $_w	= preg_replace( '/(^|\s+)(?:img|quote|code|html|script|javascript|a href|color|span|div|border|style)(\s+|$)/', str_replace( "  ", " ", "$1$2" ), $_w );
                }
                if ($_w == 'specialspacerforquotedimmunephrases') $_w = array_shift($immune[1]);
                if ($isBoolean AND !strstr($_suffix[$k],'*'))
                    $_w = IPSText::mbstrtolower($_w);
				$search_term .= (count($_prefix)?$_prefix[$k]:( $andor == 'and' )?'+':'') . $_w . $_suffix[$k];
			}
			else
			{
				if( $_w )
        ...
        $search_term	= preg_replace( '/(^|\s+)(?:[+\-~<>\*])(\s+|$)/', str_replace( "  ", " ", "$1$2" ),$search_term );
		/* Remove some formatting */
		/* InnoDB Fulltext throws a syntax error if there is a + or - at the end of the search string */
		/* @see http://stackoverflow.com/a/25972465/3113864 *//*Очень умно, учитывая, что полнотекстовый поиск в innodb поддерживается строго в последних версиях mysql, а у форума в библиотеке работы с базой полнотекстовый поиск отключается насильно для innodb. Следовательно случая, когда мы ищем через match()..agains() в innodb произойти не должно, а если произойдет, то вывалится ошибка на большинстве серверов*/
		//$search_term = str_replace( array( '|', '\\', '/' ), '', $search_term );
		// | is an OR operator for sphinx - don't want to block globally //А какое тут дело до сфинкса, если у нас библиотека для mysql?


		return array( 'search_term' => trim($search_term), 'removed' => $removedTerms );
	}
