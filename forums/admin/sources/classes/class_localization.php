    ...
    */	
	...
	public function __construct( ipsRegistry $registry )
	{
		...
		/* 	%b is month abbr
			...
			%M is min 01-59
			%p is am/pm (denchu26082014: %p does not work for ru locales. so i changed %I:%M %p to %H:%M in all default values )*/ 
		//-IBR- везде включен по умолчанию формат 24ч, т.к. во многих локалях AM/PM не используется и в режиме 12ч+AM/PM будет только 12ч
		$this->time_options = array( 'JOINED' => $this->settings['clock_joined'] ? $this->settings['clock_joined'] : '%d-%B %y',
								     'SHORT'  => $this->settings['clock_short']  ? $this->settings['clock_short']  : '%b %d %Y %H:%M',
									 'LONG'   => $this->settings['clock_long']   ? $this->settings['clock_long']   : '%d %B %Y - %H:%M',
