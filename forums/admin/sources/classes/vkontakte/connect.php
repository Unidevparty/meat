        ...
        if ( $this->_userToken AND $this->_userId )
        {
            try
            {
    			$r = $this->_api->api( 'getProfiles', 'GET', array(
    			    ...
    			    'uids' => $this->_userId, 
    			    'fields' => 'uid,first_name,last_name,nickname,photo,photo_medium,photo_big,timezone,sex,activity,email' 
    			) );
		...
		if ( $_REQUEST['code'] )
		{
			...
			if ( is_string( $rToken ) )
			{
				try
				{
					...
					$_userData = array_pop( $r['response'] );
					$_userData['email'] = isset($oAuthSession['email'])?$oAuthSession['email']:FALSE;
				}
				catch( Exception $e ){}
				...
				else
				{
					...
					$toSave = array( 'core' 		 => array(  'name' 				     => $safeName,
													 		    'members_display_name'   =>  ($this->settings['auth_allow_dnames'])?$displayName:FALSE,
					...
					{
						throw new Exception( 'CREATION_FAIL_NO_MEMBERID' );
					}
					
					/* Sync up photo */
                    $this->syncMember( $memberData );
					...
					else
					{
						...
						if ( $memberData['members_display_name'] )
						{
							/* Here, so log us in!! */
							/* changed by denchu 26/12/12*/
							$r = $this->_login()->loginWithoutCheckingCredentials( $memberData['member_id'], TRUE );
						...
						{
							throw new Exception( 'CREATION_FAIL_NO_DISPLAY_NAME' );
						}
					}
				}
			}
			...
			{
				throw new Exception( 'CREATION_FAIL_NO_TOKEN' );
			}
		}
	}
