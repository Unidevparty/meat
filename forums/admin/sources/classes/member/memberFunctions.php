...
class memberFunctions
{
		...
		try
		{
			...
			else
			{
				if ( $field == 'members_display_name' )
				{
					...
					IPSMember::save( $member['member_id'], array( 'core' => array( 'members_display_name' => $name, 'members_l_display_name' => IPSText::mbstrtolower( $name ), 'members_seo_name' => $_seoName ) ) );//-IBR-мультибайт
				...
				else
				{
					...
					IPSMember::save( $member['member_id'], array( 'core' => array( 'name' => $name, 'members_l_username' => IPSText::mbstrtolower( $name ) ) ) );//-IBR-мультибайт
	...
	{
	//-IBR-проверяем на подмену похожих символов в имени пользователя на кириллицу/латиницу
		...
		$this->DB->build( array( 
									...
									'from'   => 'members',
									'where'  => $checkField . "='" . $this->DB->addSlashes( IPSText::mbstrtolower($name) ) . "'" . ( $member['member_id'] ? " AND member_id != " . $member['member_id'] : '' ),//-IBR-мультибайт
									'limit'  => array( 0,1 ) ) );
    	...
    	//-IBR-проверка на подмену кириллицы на латиницу и наоборот  
		if ($namecyr!=$name){
			$this->DB->build( array( 
										'select' => "{$field}, member_id",
										'from'   => 'members',
    	...
    	{ 
    		$check_name = $this->DB->buildAndFetch( array( 'select' => "{$field}, member_id",
																	'from'   => 'members',
																	'where'  => "members_l_username='" . $this->DB->addSlashes( IPSText::mbstrtolower($name) ) . "'",//-IBR-мультибайт
																	'limit'  => array( 0,1 ) ) );
    	...
    	if ( $field == 'name' AND $this->settings['auth_dnames_nologinname'] )
    	{
    		$check_name = $this->DB->buildAndFetch( array( 'select' => "{$field}, member_id",
																	'from'   => 'members',
																	'where'  => "members_l_display_name='" . $this->DB->addSlashes( IPSText::mbstrtolower($name) ) . "'",//-IBR-мультибайт
																	'limit'  => array( 0,1 ) ) );
		...
		if ( $unicodeName != $name )
		{
			...
			$this->DB->build( array( 'select' => "members_display_name, member_id, email",
										   'from'   => 'members',
										   'where'  => $checkField . "='". $this->DB->addSlashes( IPSText::mbstrtolower($unicodeName) ) . "'" . ( $member['member_id'] ? " AND member_id != " . $member['member_id'] : '' ),//-IBR-мультибайт
										   'limit'  => array( 0,1 ) ) );
