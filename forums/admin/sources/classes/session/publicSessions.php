...
class publicSessions extends ips_MemberRegistry
{
	...
	public function __construct( $noAutoParsingSessions=false )
	{
		...
		if ( ( IPS_IS_AJAX && ( $this->request['section'] != 'login' && $this->request['section'] != 'skin' && $this->request['do'] != 'editBoxShow') ) //-IBR-отключаем отправку заголовков при показе свтроенной формы редактирования сообщений по аяксу. На некоторых серверах с жтим проблемы, сбивается сессия.
