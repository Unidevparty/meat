		...
		switch( $tag )
		{
			...
			case 'keywords':
				...
				if ( $encode === TRUE )
				{
					...
					$content = str_replace( array( '?','.', '!', ':', ';', ',', '"', '@', '%', '*', '(', ')' ), '', preg_replace( "/&([^;]+?);/", "", $content ) );//-IBR- ? тоже никчему в keywords
					...
					if ( is_array( $_vals ) )
					{
						foreach( $_vals as $_v )
						{
							if ( IPSText::mbstrlen( $_v ) >= 3 AND ! in_array( $_v, array_values( $_fvals ) ) AND ! in_array( $_v, $_sw ) )//-IBR-Мультибайт, чтоб не лезли на, в, над, под, за, от и т.д.
							{
								$_fvals[] = $_v;
							}
