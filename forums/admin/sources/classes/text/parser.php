...
class classes_text_parser
{
	...
	public function display( $html )
	{
		...
		{
			$html = $bbcodeParser->toHtml( $html );//-IBR- исправление бага с потерей параметров касса выставленных ранее (в оригинале класс перезапускался)
		}
	...
	protected function _seoAcronymExpansion( $txt )
	{
		...
		if ( is_array($acronyms) && count($acronyms) )
		{
			foreach( $acronyms as $r )
			{
				...
				/*																								vv Ticket #835804 */
				$wordModifier	= ( IPS_DOC_CHAR_SET == 'UTF-8' && IPSText::isUTF8( $txt ) ) ? '\-_&;\p{L}\d' : '\-\w_&;'; //-IBR- исправляем регулярку, чтоб работало там, где \w не включает русские буквы
				$caseModifier	= empty($r['a_casesensitive']) ? 'i' : '';
				$r['a_short']	= preg_quote( $r['a_short'], "/" );
				$caseModifier	.= ( IPS_DOC_CHAR_SET == 'UTF-8' && IPSText::isUTF8( $txt ) ) ?'u':''; //-IBR- включаем unicode если используется utf-8
