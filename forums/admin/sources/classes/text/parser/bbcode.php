...
class class_text_parser_bbcode extends classes_text_parser
{
	...
	protected function _finishUrlsForDisplay( $txt )
	{
		...
		preg_match_all( '#<a\s+?(?:[^>]*?)href=["\']([^"\']+?)?["\']([^>]*?)?>(.+?)</a>#isu', $txt, $urlMatches );//-IBR- поддержка unicode
		...
		for( $i = 0 ; $i < count( $urlMatches[0] ) ; $i++ )
		{
			...
			if ( $done !== true )
			{
				...
				if ( ( empty( $this->settings['__noTruncateUrl'] ) ) and IPSText::mbstrlen( $text ) > 38 && ( ! preg_match( '#&\#([0-9]{2,4})#i', $text ) ) and ( substr( $text, 0, 7 ) == 'http://' or substr( $text, 0, 8 ) == 'https://' ) )
				{
					...
					$text = htmlspecialchars( IPSText::mbsubstr(  $this->utf8_urldecode( $text ) , 0, 20 ) ) . '...' . htmlspecialchars( IPSText::mbsubstr($this->utf8_urldecode( $text ) , -15 ) ); //-IBR- добавлена функция рекодирования title, чтобы вычистить из него символы в неправильной кодировке 
				}
