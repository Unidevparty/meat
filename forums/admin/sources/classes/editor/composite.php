...
class classes_editor_composite
{
	...
	*/
	...
	{
		$content = preg_replace( '#<a\s+?([^>]*?)\shref=(["\'])(.*?)\2([^>]*?)>(.+?)<\s*/\s*a\s*>#is', "<a data-ipb='nomediaparse' $1 href='$3'$4>$5</a>" , $content );//-IBR- кто-то в IPS явно не понимает регекспов. [^data-ipb] исключает d,a,t,-,i,p,b и так далее, а не всю фразу. к тому же могут быть поменяны местами аргументы
