...
class classes_tags_search_sphinx
{
	...
	public function run( array $tags, array $options )
	{
		...
		$this->member->perm_id_array[] = 'all';//-IBR-добавляем all(мы на него заменили * при формировании индекса), чтобы искать в правах "для всех"
		/* Did we add in perm check? */
		if ( ! empty( $options['isViewable'] ) )
		{
			$query .= ' @tag_perm_text "group'.implode( '" | "group', $this->member->perm_id_array ).'"'; //-IBR-добавляем ищем не по цифрам, а по словам. для этого мы при построении индека добавли group к цифрам
