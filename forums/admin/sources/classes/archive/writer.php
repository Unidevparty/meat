...
class classes_archive_writer
{
	...
	public function __construct()
	{
		...
		{
			$this->error( $this->lang->words['archive_fail_engine'].'-'.$testresult );//-IBR-более детальная ошибка
		}
	}
	...
	public function processBatch( $options=array() )
	{
		...
		if ( $options['process'] > 0 )
		{
			...
			$this->DB->build( array( 'select' => '*',
									 ...
									 //'where'  => $query . ' AND (posts + topic_deleted_posts + topic_queuedposts ) > 0',
									 'order'  => 'tid ASC',
									 'limit'  => array( 0, $options['process'] ) ) );//учет  настройки по количеству обрабатываемых тем
