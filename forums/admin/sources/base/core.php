...
EOF;
	...
	static public function buildFurlTemplates()
	{
		...
		{//-IBR- включение возможности использования ЧПУ без заголовков(только ID)
...
class IPSDebug
{
	...
	static public function hexdump( $data, $htmloutput = true, $uppercase = false, $return = false )
	{
		...
		for( $i = $j = 0 ; $i < $len ; $i ++ )
		{
			...
			if ( ord( $data[$i] ) >= 32 )
			{
				$ascii .= ( $htmloutput === true ) ? htmlentities( $data[$i], ENT_COMPAT | ENT_HTML401, ( defined( 'IPS_DOC_CHAR_SET' ) )?IPS_DOC_CHAR_SET:ipsRegistry::$settings['gb_char_set'] ) : $data[$i]; //-IBR-на php<5.4 htmlenteties вызывает проблемы с кириллицей в utf-8
			}
			else
			{
				$ascii .= '.';
...
class IPSText
{
	 ...
	 * -IBR-Функция перевода месяцев на английский для передачи strttime  
	 * @access	public
	 * @param	string		Date
	...
	{
		return str_ireplace( array( '%D0%B0', '%D0%B1', '%D0%B2', '%D0%B3', '%D0%B4', '%D0%B5', '%D1%91', '%D0%B6', '%D0%B7', '%D0%B8', '%D0%B9', '%D0%BA', '%D0%BB', '%D0%BC', '%D0%BD', '%D0%BE', '%D0%BF', '%D1%80', '%D1%81', '%D1%82', '%D1%83', '%D1%84', '%D1%85', '%D1%86', '%D1%87', '%D1%88', '%D1%89', '%D1%8D', '%D1%8E', '%D1%8F', '%D1%8B', '%D1%8C', '%D1%8A', '%D0%90', '%D0%91', '%D0%92', '%D0%93', '%D0%94', '%D0%95', '%D0%81', '%D0%96', '%D0%97', '%D0%98', '%D0%99', '%D0%9A', '%D0%9B', '%D0%9C', '%D0%9D', '%D0%9E', '%D0%9F', '%D0%A0', '%D0%A1', '%D0%A2', '%D0%A3', '%D0%A4', '%D0%A5', '%D0%A6', '%D0%A7', '%D0%A8', '%D0%A9', '%D0%AD', '%D0%AE', '%D0%AF', '%D0%AB', '%D0%AC', '%D0%AA', 'а', 'б', 'в', 'г', 'д', 'е', 'ё', 'ж', 'з', 'и', 'й', 'к', 'л', 'м', 'н', 'о', 'п', 'р', 'с', 'т', 'у', 'ф', 'х', 'ц', 'ч', 'ш', 'щ', 'э', 'ю', 'я', 'ы', 'ь', 'ъ' ), array('a', 'b', 'v', 'g', 'd', 'e', 'yo', 'zh', 'z', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'r', 's', 't', 'u', 'f', 'kh', 'tc', 'ch', 'sh', 'sch', 'e', 'iu', 'ia', 'y', '', '', 'A', 'B', 'V', 'G', 'D', 'E', 'YO', 'ZH', 'Z', 'I', 'I', 'K', 'L', 'M', 'N', 'O', 'P', 'R', 'S', 'T', 'U', 'F', 'KH', 'TC', 'CH', 'SH', 'SCH', 'E', 'IU', 'IA', 'Y', '', '', 'a', 'b', 'v', 'g', 'd', 'e', 'yo', 'zh', 'z', 'i', 'i', 'k', 'l', 'm', 'n', 'o', 'p', 'r', 's', 't', 'u', 'f', 'kh', 'tc', 'ch', 'sh', 'sch', 'e', 'iu', 'ia', 'y', '', ''), $text);
	...
	static public function makeSeoTitle( $text )
	{
		...
		/* Finish off */
		$text = self::mbstrtolower($text); //-IBR-Мультибайт
		...
		$text = self::makeSeoTransliterate( $text ); //-IBR- Транслитерация FURL
	...
	static public function removeControlCharacters( $t )
	{
    	...
    	if ( ! isset( ipsRegistry::$settings['strip_space_chr'] ) OR ipsRegistry::$settings['strip_space_chr'] )
    	{
    		 ...
    		 */
			if ( strtolower(IPS_DOC_CHAR_SET) == 'utf-8' )//-IBR- чтобы не портить UTF
			{
