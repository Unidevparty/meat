	...
	static public function init()
	{
		...
		else if ( IPS_AREA == 'public' )
		{
			...
			if ( self::$handles['member']->getProperty('member_banned') == 1 or self::$handles['member']->getProperty( 'temp_ban' ) )
			{
				...
				if ( ipsRegistry::$request['module'] != 'ajax' or (ipsRegistry::$request['section'] != 'warnings' and ipsRegistry::$request['section'] != 'notifications' )) //-IBR-чтобы заблокированный мог увидеть уведомление о блокировке
				{
...
class ips_MemberRegistry
{
	...
	static public function setSearchEngine( $uAgent )
	{
		...
		/* Are we mobile? */
		if ( $uAgent['uagent_key'] == 'googlemobile' or $uAgent['uagent_key'] == 'yandexmobile' )
		{
		...
		if ( ipsRegistry::$settings['spider_visit'] )
		{
			ipsRegistry::DB()->insert( 'spider_logs', array ( 'bot'		     => $uAgent['uagent_key'],
															  'request_addr' => htmlentities( strip_tags( str_replace( '\\', '',  str_replace( "'", "", my_getenv('REQUEST_URI')) ) ), ENT_COMPAT | ENT_HTML401, ( defined( 'IPS_DOC_CHAR_SET' ) )?IPS_DOC_CHAR_SET:ipsRegistry::$settings['gb_char_set'] ),//-IBR- выставляем кодировку для php<5.4
...
class seoTracker
{
	 ...
	 * @var array
	 */
//-IBR- добавлены наши ПС и учтен https 
	protected static $engines  = array(
		array( 'name' => 'Google', 'match' => '/https?\:\/\/www\.google/', 'parser' => 'parseGoogleUrl' ),
		array( 'name' => 'Bing', 'match' => '/https?\:\/\/www\.bing/', 'parser' => 'parseBingUrl' ),
		array( 'name' => 'Bing', 'match' => '/https?\:\/\/www\.bing/', 'parser' => 'parseBingUrl' ),
		array( 'name' => 'Facebook', 'match' => '/https?\:\/\/www\.facebook/', 'parser' => 'parseFacebookUrl' ),
		array( 'name' => 'Yandex', 'match' => '/https?\:\/\/(www\.|hghltd\.|images\.|)yandex/', 'parser' => 'parseYandexUrl'),
		array( 'name' => 'YandexM', 'match' => '/https?\:\/\/m\.yandex/', 'parser' => 'parseYandexMUrl'),
		array( 'name' => 'Rambler', 'match' => '/https?\:\/\/(nova\.)?rambler/', 'parser' => 'parseRamblerUrl'),
		array( 'name' => 'Mail.ru', 'match' => '/https?\:\/\/(go\.)?mail/', 'parser' => 'parseMailruUrl')
	);
	...
	protected function parseYandexUrl($url)//-IBR- парсим янексовские реферреры
	{
		$matches = array();
		if(preg_match('/text\=([^\&]+)/', $url, $matches))
		{
			if(isset($matches[1]))
			{
				...
				$keywords = str_replace('+', ' ', $keywords);
				$keywords = str_replace('%20', ' ', $keywords);
				return mb_substr($keywords,0,250);
			}
		}
		
		return null;
	}
	protected function parseYandexMUrl($url)
	{
		$matches = array();
