...
class login_convert extends login_core implements interface_login
{
	...
	protected function _authenticateSmf11( $username, $password )
	{
		...
		if( $this->_memberData[ $this->password_field ] )
		{
			$username_low	= mb_strtolower($username);//-IBR- делаем имена действительно регистронезависимыми
	...
	{
		$member = $this->DB->buildAndFetch( array( 'select' => 'member_id', 'from' => 'members', 'where' => "members_l_username='" . $this->DB->addSlashes( mb_strtolower($username) ) . "'" ) );//-IBR- мультибайт
