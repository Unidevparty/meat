...
class login_external extends login_core implements interface_login
{
	...
	public function authenticate( $username, $email_address, $password )
	{
		...
		//-----------------------------------------
		if ($username) { //-IBR-добавляем возможность аутентификации по e-mail
	...
	{
		$member = $this->DB->buildAndFetch( array( 'select' => 'member_id', 'from' => 'members', 'where' => "members_l_username='" . mb_strtolower($username) . "'" ) );//-IBR-мультибайт
