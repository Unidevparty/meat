...
class search_format_ccs extends search_format
{
	...
	public function parseAndFetchHtmlBlocks( $rows )
	{
		...
		foreach( $rows as $id => $data )
		{
			...
			if( strpos( $html, '[attachment=' ) !== false )
			{
				...
				{
					$_attach	= $this->DB->buildAndFetchAll( array( 'select' => 'attach_rel_id', 'from' => 'attachments', 'where' => 'attach_id IN (' . implode(',', $attach_ids) .')' ), 'attach_rel_id' );
					
					if( is_array($_attach) and count($_attach) )
					{
						...
						$_attachments->type		= 'ccs';
						$_attachments->init();

						$attachHTML = $_attachments->renderAttachments( $html, array_keys($_attach), 'ccs_global' );
				
						/* Now parse back in the rendered posts */
						if( is_array($attachHTML) AND count($attachHTML) )
						{
							foreach( $attachHTML as $_data )
							{
