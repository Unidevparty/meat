...
{	
	...
	public static function canTrack( $member, $function, $functionId = 0 )
	{
		...
		/* Still in time to be tracked? */
        if ( time() >= self::getMember( 'member_tracked_deadline' ) AND self::getMember( 'member_tracked_deadline' ) > 0 )
        {
        	IPSMember::save( self::getMember( 'member_id' ), array( 'core' => array( 'member_tracked' => 0 ) ) );
	...
	private static function checkFunction( $function )
	{
		...
		if ( isset( $curSettings[ $function ] ) AND $curSettings[ $function ] == 0 )
		{
			return false;
		}
