 ...
 * Last Updated: $Date: 2014-03-03 14:28:30 +0000 (Mon, 03 Mar 2014) $
 * </pre>
 *
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		if ( $this->request['url'] AND $this->request['title'] )
		{
			...
			if ( ! empty( $this->settings['ccs_root_url'] ) )
			{
				...
				{
					if ( ! empty($_url) AND substr( $this->page['url'], 0, strlen( $_url ) ) === $_url )
					{
						$ok = TRUE;
						break;
					}
			...
			{
				if ( substr( $this->page['url'], 0, strlen( $this->settings['_original_base_url'] ) ) !== $this->settings['_original_base_url'] ) 
				{
