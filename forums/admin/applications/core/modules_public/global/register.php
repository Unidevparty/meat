...
class public_core_global_register extends ipsCommand
{
	...
	public function _completeRegistrationSave()
	{
		...
		if ( $connectService AND $login_user AND $login_pass AND ( $this->settings['fb_realname'] == 'enforced' OR ! $members_display_name ) )
		{
			...
			else
			{
				...
				else if ( $connectService == 'vkontakte' AND $member['vk_uid'] )//-IBR- для вк
				{
					/* Already VKd? */
		...
		else
		{
			IPSMember::save( $mid, array( 'members' => array(
																...
																'members_l_username'		=> $tmp_member['members_l_username'] ? $tmp_member['members_l_username'] : $members_display_name,
																'member_group_id'			=> $member_group_id,
																'members_l_display_name'	=> IPSText::mbstrtolower( $members_display_name )//-IBR-Мультибайт, для русских имен
							)			)	);
		}
	...
	protected function _completeRegistration( $form_errors=array() )
	{
			...
			if ( $userData['id'] )
			{
				...
				if ( ! count( $form_errors['dname'] ) AND $this->settings['fb_realname'] != 'any' )
				{
					...
					$max = $this->DB->buildAndFetch( array( 'select' => 'MAX(member_id) as max',
													 		'from'   => 'members',
													 		'where'  => "members_l_display_name LIKE '" . $this->DB->addSlashes( IPSText::mbstrtolower( $userData['_displayName'] ) ) . "%'" ) );//-IBR-Мультибайт, для русских имен
		...
		else if ( $member['vk_uid'] AND $member['vk_token'] )//-IBR-для ВК
		{
			/* Attempt to fetch user details */
			$classToLoad	= IPSLib::loadLibrary( IPS_ROOT_PATH . 'sources/classes/vkontakte/connect.php', 'vkontakte_connect' );
	...
	protected function _revalidateComplete()
	{
		...
		if ( $val['lost_pass'] == 1 )
		{
			...
			//-IBR-Добавлена поддержка ЧПУ в письме с активацией. Это позволяет решить проблему с &sect	
 	...
 	protected function registerCoppaTwo()
	{
		...
		$birthday = strtotime( $this->request['month'] ."/" . $this->request['day'] . "/" . $this->request['year']); //-IBR- заменили - на / для того, чтобы дата воспринималась в стандарте http://php.net/manual/en/function.strtotime.php (см. предпоследнее примечание)
	...
	protected function _autoValidate()
 	{
		...
		else if ( $validate['lost_pass'] == 1 )
		{
			...
			//-IBR-Поддержка ЧПУ в письмах для решения проблемы с &sect	
			IPSText::getTextClass('email')->buildMessage( array(
															'NAME'		=> $member['members_display_name'],
			...
			if( $data['qa_id'] )
			{
				...
				if( count($answers) )
				{
					foreach( $answers as $answer )
					{
						...
						if( IPSText::mbstrlen($answer) AND IPSText::mbstrtolower($answer) == IPSText::mbstrtolower($this->request['qa_answer']) )//-IBR- мультибайт, чтобы ответ был действительно регистронезависимым, а не как в оригинале
						{
							$pass	= true;
							break;
						}
