    ...
    public function loginForm( $message="", $replacement='' )
    {
		...
		if ( $serviceClick )
		{
			switch ($serviceClick)
			{
				...
				break;
				case 'vkontakte':
				    $this->registry->getClass('output')->silentRedirect( $this->settings['_original_base_url'] . "/interface/vkontakte/index.php?reg=1" );//-IBR-для вконтакте
				break;
			}
		}
		...
		if ( $show_form )
		{
			...
			$template .= $this->registry->getClass('output')->getTemplate('login')->showLogInForm( $this->lang->words['please_log_in'], htmlentities(urldecode($http_referrer), ENT_COMPAT | ENT_HTML401, ( defined( 'IPS_DOC_CHAR_SET' ) )?IPS_DOC_CHAR_SET:ipsRegistry::$settings['gb_char_set']), $extra_form, $login_methods );//-IBR-в php 5.3 и ниже htmlentities будет портить русские буквы
		}
