...
//-IBR-этот фрагмент решает проблему с $sect в урлах на подтверждение мыла, смену пароля и т.д.
...
'section=rss'      => array( 'app'		     => 'core',
							 'allowRedirect' => 0,
							 'out'           => array( '#app=core(&amp;|&)module=global(&amp;|&)section=rss(&amp;|&)type=(\w+?)$#i', 'rss/$4/' ),//-IBR-это исправляет баг с нерабочими ссылками в rss, убрана путаница с blog/rss
...
'section=rss2'     => array( 'app'		     => 'core',
						     ...
						     'in'            => array( 'regex'   => "#^/rss/(\w+?)/(\w+?)(?:-|/|$)#i",//-IBR-это исправляет баг с нерабочими ссылками в rss, убрана путаница с blog/rss
