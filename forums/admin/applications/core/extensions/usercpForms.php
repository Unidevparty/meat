...
//-IBR-чтобы работало вконтакте
	 ...
	 * //-IBR-функция удаления связи аккаунта с вконтакте
	 * @return	@e void  
	 */
	...
	public function showForm( $current_area, $errors=array() )
	{
		...
		switch( $current_area )
		{
			...
			break;
			case 'vkontakte':
				return $this->formVkontakte();//-IBR-вконтакте
			break;
			case 'customize':
	 ...
	 * -IBR- Показывает форму связи аккаунта с ВК
	 * @author	
	 * @param	string		Any inline message to show
	 * @return	string		Processed HTML
	 */
	...
	public function saveSignature()
	{
		...
		if( isset($sig_restrictions[4]) and $sig_restrictions[4] !== '' )
		{
			...
			{
				preg_match_all( '#(^|\s|>|url\s*=\s*[\'"]?)((http|https|news|ftp)://\w+[^\s\[\]\<]+)#is', $signature, $matches );//-IBR-добавили учет пробелов внутри тега, учет ббкода
	...
	//-IBR-добавлена поддержка FURL в ссылках на смену e-mail, чтобы решить проблему с $sect
