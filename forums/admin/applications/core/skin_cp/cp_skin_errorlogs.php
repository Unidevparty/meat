...
if( count( $rows ) AND is_array( $rows ) )
{
	foreach( $rows as $row )
	{
		$row['log_request_uri']	= htmlentities( $row['log_request_uri'], ENT_QUOTES, ( defined( 'IPS_DOC_CHAR_SET' ) )?IPS_DOC_CHAR_SET:ipsRegistry::$settings['gb_char_set'] );//-IBR- Как обычно, без указаниякодировки будут проблемы на php < 5.4
