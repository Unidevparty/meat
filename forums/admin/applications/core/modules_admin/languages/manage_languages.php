...
class admin_core_languages_manage_languages extends ipsCommand
{
			...
			try
			{
				foreach( new DirectoryIterator( $_PATH ) as $f )
				{
					if ( preg_match( "#" . $_word . "_(.+?)_language_pack.xml#", $f->getFileName() ) )
					{
						...
						$this->imprtFromXML( 1, true, true, $next['key'], false, 1 ); //-IBR-Импорт word_custom
					}
				}
			} catch ( Exception $e ) {}
	 ...
	 * @return	mixed
	 */
	public function imprtFromXML( $lang_id=0, $in_dev=0, $no_return=0, $app_override='', $skip_charset=false, $ru = '' ) //-IBR-Добавлен параметр для импорта word_custom
	{
		//-----------------------------------------
		// Set version..
		//-----------------------------------------
		...
		foreach( $xml->fetchElements('lang') as $entry )
		{
			...
			foreach( $lang_ids as $_lang_id )
			{
				...
				$db_array = array(
									...
									'word_custom'			=> $in_dev && !$ru ? '' : $lang['word_custom'],//-IBR-интерпретация параметра для импорта word_custom
									'word_js'				=> intval($lang['word_js']),
									'word_default_version'	=> isset($lang['word_default_version']) ? $lang['word_default_version'] : $LATESTVERSION['long'],
									'word_custom_version'	=> isset($lang['word_custom_version']) ? $lang['word_custom_version'] : $LATESTVERSION['long'],
								);
				...
				if ( isset( $lang['word_default']) AND md5( $lang['word_default'] ) == $lang_entries[ $dbKey ]['word_default'] )
				{
				...
				if ( isset($lang_entries[ $langIdKey ]) AND $lang_entries[ $langIdKey ] )
				{
				...
				if ( isset($lang_entry['word_id']) AND $lang_entry['word_id'] AND ( ! $insertId OR ( $insertId == $_lang_id ) ) )
				{
					//-----------------------------------------
					// Don't update default version
					//-----------------------------------------
				...
				else if( !isset($lang_entry['word_id']) OR !$lang_entry['word_id'] )
				{
	...
	public function languageExportToXML( $lang_id, $disk=false )
	{
		...
		while( $r = $this->DB->fetch() )
		{
			...
			if( $disk )
			{
				...
				if( $r['word_custom'] )//-IBR- Экспорт word_custom
				{
