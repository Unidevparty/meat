...
class admin_core_seo_activity extends ipsCommand
{
	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		{
			$this->engine = $this->DB->addSlashes( mb_strtolower( $this->request['engine'] ) );//-IBR- муьтибайт забыли, вдруг ПС зовут по-русски
	...
	protected function _showKeywords()
	{
		...
		{
			$row['keyword'] = IPSText::parseCleanValue( mb_strtolower( $row['keyword'] ) );//-IBR- муьтибайт для русских поисковых запросов
			$keywords[] = $row;
		}
	...
	protected function _showVisitors()
	{
		...
		if ( $max['total'] )
		{
			...
			while ( $row = $this->DB->fetch( $e ) )
			{
				...
				$row['keywords']	= IPSText::parseCleanValue( urldecode( mb_strtolower( $row['keywords'] ) ) );//-IBR- муьтибайт для русских поисковых запросов
	...
	private function _buildChart( $startTime )
	{
		...
		if ( $this->days == 1 )
		{
			...
			{
				$labels[] = date( 'H', IPSTime::getTimestamp() );//-IBR-переделали из 12AM в 24. В русской локали 12AM и 12PM не отличаются(am/pm просто съедается)
			}
		}
		else
		{
			...
			{
				$labels[] = strftime( '%e %b', IPSTime::getTimestamp() ); //-IBR-Чтобы месяц был по-русски берем его из локали date() всегда возвращает английский.
			}
		}
