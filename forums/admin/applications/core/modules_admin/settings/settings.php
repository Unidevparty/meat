...
class admin_core_settings_settings extends ipsCommand
{
	...
	public function _findSettingGroup()
	{
		...
		while( $r = $this->DB->fetch() )
		{ 	
			if(  ( $r['conf_title_keyword'] == $this->request['key'] ) OR IPSText::mbstrtolower( str_replace( " ", "", trim( $r['conf_title_title'] ) ) ) == urldecode( trim( $this->request['key'] ) ) )//-IBR-Мультибайт, чтобы искать в русских названиях настроек
			{
	...
	public function _viewSettings()
	{
		...
		if( $search_key )
		{
			$keywords	= IPSText::mbstrtolower($search_key);//-IBR-Мультибайт, чтобы искать в русских названиях настроек
	...
	protected function _settingsTitlesCheck()
	{
		...
		while( $row = $this->DB->fetch( $outer ) )
		{
			...
			{
				$new_keyword = IPSText::mbstrtolower( preg_replace( '#[^\d\w]#', "", $row['conf_title_title'] ) );//-IBR-Мультибайт, чтобы искать в русских поисковых ключах
