...
class admin_core_ajax_livesearch extends ipsAjaxCommand 
{
	...
	protected function _getMembers( $term, $results )
	{
		...
		$term	= IPSText::mbstrtolower($term); //-IBR-чтобы работало с кириллицей UTF8
	...
	protected function _checkGroups( $term, $results )
	{
		...
		$term = IPSText::mbstrtolower($term);//-IBR-чтобы работало с кириллицей(мультбайт)
		...
		if ( is_array( $groups ) AND count( $groups ) )
		{
			foreach( $groups as $id => $data )
			{
				...
				if ( preg_match( "#" . $_term . "#iu", $data['g_title'] ) )//-IBR-включили поддержку unicode
				{
					$results['groups'][] = array(
	...
	protected function _checkForums( $term, $results )
	{
		...
		$term				= IPSText::mbstrtolower($term);//-IBR-включили мультибайт
		...
		foreach( $forums->forum_by_id as $id => $data )
		{
			...
			if ( preg_match( "#" . $_term . "#iu", $data['name'] ) )//-IBR- включили unicode
			{
	...
	protected function _checkNexus( $term, $results )
	{
		...
		$textTerm		= $this->DB->addSlashes( IPSText::mbstrtolower( $term ) );//-IBR-чтоб работало с мультибайтом
		$intTerm		= intval( $term );
