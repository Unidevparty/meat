...
class admin_core_applications_hooks extends ipsCommand
{
	...
	public function installHook( $content, $addMessage=FALSE, $allowSkinRecache=TRUE, $enabled=1 )
	{
		...
		foreach( $xml->fetchElements('hookextras_replacements') as $node )
		{
			foreach( $xml->fetchElements('replacements', $node) as $_r )
			{
				...
				$replacements[]	= array(
									...
									'replacement_master_key' => $_r['replacement_master_key'], //-IBR-баг у IPS не импортирует макросы хука
									);
			}
		}
		...
		if( count( $replacements ) )
		{
			...
			{
				if ( !isset( $entry['replacement_master_key']  ) or !isset( $bits[ $entry['replacement_master_key'] ]  ) )//-IBR- Баг IPS не позволял импортировать макросы при импорте хука
				{
