	...
	public function doExecute( ipsRegistry $registry )
	{
		...
		foreach( $mapit as $in => $tbl )
		{
			...
			if ( $in == 'name' and $inbit != "" )
			{
				...
				{
					$query[] = "m.members_l_display_name LIKE '" . IPSText::mbstrtolower( $inbit ) . "%'";//-IBR- мультибайт
				}
				else
				{
					$query[] = "m.members_l_display_name LIKE '%" . IPSText::mbstrtolower( $inbit ) . "%'";//-IBR- мультибайт
			...
			else if ( in_array( $in, $dates ) and $inbit )
			{
				...
				{
					$time_int 	= strtotime( IPSText::monthNameRu2En($this->request[ $inbit ]) );//-IBR-перевод месяца
				}
