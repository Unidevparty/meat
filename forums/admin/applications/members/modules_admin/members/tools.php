...
class admin_members_members_tools extends ipsCommand
{
	...
	protected function _showIPs()
	{
		...
		{
			$member = $this->DB->buildAndFetch( array( 'select' => 'member_id, members_display_name, email, ip_address', 'from' => 'members', 'where' => "members_l_username='" . $this->DB->addSlashes( IPSText::mbstrtolower($this->request['name']) ) . "' OR members_l_display_name='" . $this->DB->addSlashes( IPSText::mbstrtolower($this->request['name']) ) . "'" ) );//-IBR- мультибайт
