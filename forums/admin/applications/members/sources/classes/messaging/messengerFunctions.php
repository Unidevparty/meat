...
class messengerFunctions
{
	...
	public function checkAndReturnInvitedUsers( $names, $member=array(), $current=0 )
	{
		...
		foreach( $_members as $member_id => $_member )
		{
			...
			$inviteArrayByName[ IPSText::mbstrtolower($_member['members_l_display_name']) ] = $_member['member_id'];//-IBR- мультибайт чтобы уж точно low name было в нижнем регистре
		...
		foreach( $names as $name )
		{
			...
			if ( ! in_array( IPSText::mbstrtolower( $name ), array_keys( $inviteArrayByName ) ) )//-IBR- мультибайт, чтобы можно было пригласить юзера с русским ником
			{
				$doesNotExist[] = $name;
			}
		}
