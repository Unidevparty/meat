...
class public_forums_forums_forums extends ipsCommand
{
	...
	public function renderForum()
	{
		...
		$sort_by		= $this->selectVariable( array(
												...
												3 => !empty($this->forum['sort_order'])?$this->forum['sort_order'] : NULL, //-IBR-позже его проверят на isset и окажется, что там пустая строка, потому надо поставиь NULL, что не было
												4 => 'Z-A'				      )
									   );
    ...
    public static function selectVariable($array)
    {
    	...
    	foreach ($array as $v)
    	{
    		if ( isset($v) && !empty($v) )//-IBR- тут повторное устранение того же бага. при пустом значении фильтра выдавалась ошибка
    		{
    			$chosen = $v;
    			break;
    		}
