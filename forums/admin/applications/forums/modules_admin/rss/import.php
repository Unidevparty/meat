...
class admin_forums_rss_import extends ipsCommand
{
	...
	public function rssImportSave($type='add')
	{
		...
		/* Member data */
		$member = $this->DB->buildAndFetch( array( 'select' => 'member_id, name', 'from' => 'members', 'where' => "members_l_display_name='" . IPSText::mbstrtolower( $rss_import_mid ) . "'" ) );//-IBR- мультибайт для имени пользователя
			...
			foreach ( $this->class_rss->rss_channels as $channel_id => $channel_data )
			{
				...
				{			
					...
					foreach( $this->class_rss->rss_items[ $channel_id ] as $item_data )
					{
						...
						/*- IBR - Руские буквы в title RSS2 как правило закодированы в entities*/
                        $item_data['title'] = html_entity_decode($item_data['title'], ENT_QUOTES, 'UTF-8');
