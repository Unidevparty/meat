...
class admin_forums_archive_archive extends ipsCommand
{
	...
	protected function _archiveDash()
	{
		...
		if ( $restoreData['restore_min_tid'] AND $restoreData['restore_max_tid'] )
		{
			...
			{
				$tmp = $topicClass->getTopics( array( 'archiveState' => array( 'archived' ), 'dateIsGreater' => $date, 'tidIsGreater' => ( $restoreData['restore_min_tid'] - 1 ), 'tidIsLess' => ( $restoreData['restore_max_tid'] + 1 ), 'limit' => (10 - count( $restoreTopics ) ) ) );//-IBR- у ips опечатка. чтобы выбрать от мин до мах включительно, то надо к мах прибавлять 1
			}
