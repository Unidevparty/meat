...
<div class="row">
  ...
  <div class="col-xs-6 col-sm-4 col-md-3 col-lg-2 img-row">
    ...
    <div class="ssquare file-item clickable {{ $item->is_file ? '' : 'folder-item' }}" style="width: 200px; height: 200px" data-id="{{ str_replace('laravel-filemanager/', '', $item_path) }}"
           @if($item->is_file && $thumb_src) onclick="fileView('{{ $item_path }}', '{{ $item->updated }}')"
           @elseif($item->is_file) onclick="download('{{ $item_name }}')" @endif >
      @if($thumb_src)
      <img src="{{ $thumb_src }}" style="max-width: 100%; max-height: 100%;">
      @else
